...
class RSLog:
    ...
    def GetPlayType(self):
        ...
        if re.search('SB[23H]', playtyp) != None:
            ...
            self.currentBase.values['total_sb'] = self.currentBase.values['second_stolen'] + \
                                                  + self.currentBase.values['third_stolen'] + self.currentBase.values[
                                                      'home_stolen']
        ...
        if re.search('CS[23H]', playtyp) != None:
            ...
            self.currentBase.values['total_cs'] = self.currentBase.values['second_caught'] + \
                                                  self.currentBase.values['third_caught'] + self.currentBase.values[
                                                      'home_caught']
    ...
    def PitchSub(self, userid, home_ind, batnum):
        IP = float(self.currentSim.inning)-1+float(self.currentSim.outs/3.0)
        for pit in self.pitroster.keys():
            if home_ind == 1 and self.pitroster[pit].values['team_key'] == self.currentGame.values['home_team_key']:
                IP -= self.pitroster[pit].values['IP']
                self.pitroster[self.currentSim.activeHomePitcher].values['IP'] = IP
                self.currentSim.activeHomePitcher = userid
            elif self.pitroster[pit].values['team_key'] == self.currentGame.values['away_team_key']:
                IP -= self.pitroster[pit].values['IP']
                self.pitroster[self.currentSim.activeAwayPitcher].values['IP'] = IP
                self.currentSim.activeAwayPitcher = userid
    ...
    def ParseLog(self):
        ...
        for line in text:
            ...
            if rowType == 'info':
                ...
                if int(row[5]) == 1:
                    self.PitchSub(row[1], int(row[3]), int(row[4]))
                elif int(row[5]) == 11:
                    self.HitSub(row[1], int(row[3]), int(row[4]), int(row[5]))
                elif int(row[5]) == 12:
                    self.RunSub(row[1], int(row[3]), int(row[4]), int(row[5]))
                else:
                    self.DefSub(row[1], int(row[3]), int(row[4]), int(row[5]))
            elif rowType == 'data':
